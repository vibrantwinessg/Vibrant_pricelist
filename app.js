...
function renderCart() {
  ...
  $("cartBody").innerHTML = detailed.map(x => `
    <div class="cart-line">
      <div class="cart-line-info">
        ...
        <div class="cart-line-meta">${escapeHtml(x.type || "")}${x.type ? " · " : ""}${money(x.price)} / bottle</div>
  ...
  const totalBottles = detailed.reduce((sum, x) => sum + x.qty, 0);
  let breakdownHtml = `<div class="cart-breakdown-line"><span>Total Bottles</span><span>${totalBottles}</span></div>`;
  breakdownHtml += `<div class="cart-breakdown-line"><span>Subtotal</span><span>${money(subtotal)}</span></div>`;
  if (fee > 0) {
...
function renderOrderSummary() {
  ...
  let html = detailed.map(x => `
      <div class="order-summary-line">
        <span>${escapeHtml(x.producer)} — ${escapeHtml(x.cuvee)}: ${money(x.price)} &times; ${x.qty}</span>
        <span>${money(x.lineTotal)}</span>
      </div>`).join("");
  html += `<div class="order-summary-line"><span>Total Bottles</span><span>${totalBottles}</span></div>`;
...
async function submitOrder(e) {
  ...
  const fee = deliveryFee();
  const totalBottles = detailed.reduce((sum, x) => sum + x.qty, 0);
  const itemsText = detailed.map(x => `${x.producer} — ${x.cuvee} (${x.type || ""}): ${money(x.price)} x ${x.qty} = ${money(x.lineTotal)}`).join("\n")
    + `\n\nTotal Bottles: ${totalBottles}`
    + `\nSubtotal: ${money(subtotal)}`
    + (fee > 0 ? `\nDelivery fee: ${money(fee)}` : `\nDelivery: Free`);
  const itemsHtml = detailed.map(x => `<div>${escapeHtml(x.producer)} — ${escapeHtml(x.cuvee)} (${escapeHtml(x.type || "")}): ${money(x.price)} &times; ${x.qty} = ${money(x.lineTotal)}</div>`).join("")
    + `<div><br>Total Bottles: ${totalBottles}</div>`
