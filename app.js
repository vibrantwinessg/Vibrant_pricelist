...
let activeFilters = { search: "", region: "", style: "", grape: "", producer: "", sort: "default" };
...
function wireStaticEvents() {
  ...
  $("resetFilters").addEventListener("click", () => {
    activeFilters = { search: "", region: "", style: "", grape: "", producer: "", sort: "default" };
    $("searchInput").value = "";
...
function renderCart() {
  ...
  breakdownHtml += `<div class="drawer-total"><span>Total</span><strong>${money(cartGrandTotal())}</strong></div>`;
...
function renderOrderSummary() {
  ...
  html += `<div class="order-summary-total"><span>Total</span><span>${money(cartGrandTotal())}</span></div>`;
...
async function submitOrder(e) {
  ...
  const totalStr = money(cartGrandTotal());
